> 2026-09-19 手机工作室已部署至 https://edit.a4.fit 。当前证据、登录说明位置和未完成项以 [交代文档](docs/handoff-mobile-workspace.md) 为准；下文暂停平台的描述属于历史阶段。
...
当前优先使用手机工作室：上传素材、提交剪辑、查看工程和版本、播放下载。已有本地 video-2022 交付适配器仍保留。

当前已从需求文档补到**可运行的开源引擎验证工作台**：素材探测 → 结构化方案 → 覆盖校验 → 中文字幕/转场/声音处理 → MP4 与质量报告。手机页面、持久队列、订阅执行器及私有OSS交付现已部署；实际证据和限制见顶部交代文档。
...
原素材和成片不提交 Git。用户已有火山方舟订阅与 DeepSeek API key 两条编程助手通道；服务端订阅执行路线要单独验证，不能把付费 API 当渲染默认前提。优先开源复用，不先重写完整编辑器。
