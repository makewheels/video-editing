...
## 登录失效的接续经验（2026-09-18）

401不能直接等同于必须让用户手工登录。先检查点播项目当前CLI文档、已有profile及是否有正式支持的恢复方式；已有用户授权时自行完成。此次用户明确授权自行登录，按该项目已记录的测试账号流程恢复并验证成功；没有读取浏览器凭据或输出令牌。测试流程不能假定在其他部署同样有效。

若401发生在create_pending阶段，先核对平台是否存在本次视频。此次明确收到创建接口401且列表无记录，因此保留原失败回执后重新交付；不要把“创建结果未知”和“明确未授权失败”混为一谈。若服务器是否创建仍不明确，继续按现有防重规则恢复ID，不盲目新建。
