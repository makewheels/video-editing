...
简洁字幕与压缩配置继续复用 FFmpeg、Pillow、固定提交的 vedit，无需新增识别模型、付费模型 API、OpenCV 或 ASR。动作识别仍依赖连续画面审核。

授权交付增加可选 `delivery` extra（truststore 使用系统证书），通过 `uv --with /path/to/video-2022/cli` 复用现有 CLI 及其 requests/oss2，不复制账号系统。仅本地剪辑无需安装此 extra。具体命令、环境代理、上传失败恢复和临时缓存清理见 [交付手册](delivery.md)。不关闭 TLS 验证，不把本机代理地址写进程序。

原生 Windows 已实际完成渲染与上传。没有创建符号链接的权限时相应用例跳过，路径逃逸测试仍执行。大型模型、队列和手机产品工具继续按明确需求评估，不因一次修改自动安装。
